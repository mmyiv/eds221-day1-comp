...
```{r}
gg1save(here::here("figs", "toolik_temp_plot.png"), height = 6, width = 7)
```
...
```{r}
toolik_chorophyll <- read_csv(here::here("data", "data-raw", "toolik_chlorophyll.csv"))
```

```{r}
skimr::skim(toolik_chorophyll)
```

```{r}
toolik_chlorophyll <- toolik_chorophyll %>% clean_names()
```

```{r}
dim(toolik_chorophyll)
names(toolik_chorophyll)
```

```{r}
# Turn periods from data in NA. Store the variable then write
toolik_chlorophyll$active_chl_a_ug_l <- 
na_if(toolik_chlorophyll$active_chl_a_ug_l, ".")

view(toolik_chlorophyll)
```

```{r}
#update data to right variable type chr -> num
toolik_chlorophyll$active_chl_a_ug_l <- as.numeric(toolik_chlorophyll$active_chl_a_ug_l)
```

```{r}
#plot active chlr [] by depth
